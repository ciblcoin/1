...
export default function ChallengeList({ currentUserId }) {
  ...
  const fetchChallenges = async () => {
    try {
      ...
      if (data) {
        // فقط چالش‌های فعال را نمایش می‌دهیم (open یا joined)
  ...
  const handleJoinChallenge = async (challengeId) => {
    ...
    try {
      // ابتدا چالش را بررسی می‌کنیم که هنوز باز باشد
      ...
      // کاربر نمی‌تواند به چالش خودش بپیوندد
      ...
      // به‌روزرسانی چالش برای پیوستن کاربر
  ...
  // بارگیری اولیه و تنظیم شنونده Real-time
  useEffect(() => {
    fetchChallenges();

    // تنظیم کانال Real-time برای گوش دادن به تغییرات
      ...
      .on(
        ...
        async (payload) => {
          ...
          // اگر چالش به‌روزرسانی شد
            ...
            // اگر وضعیت به 'joined' تغییر کرد، اطلاعات joiner را دریافت کن
    ...
    // تمیزکردن کانال هنگام خروج از کامپوننت
    return () => {
